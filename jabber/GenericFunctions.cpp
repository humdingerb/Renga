...
string GenericFunctions::GenerateUniqueID() {
...
string GenericFunctions::TimeStamp() {
...
string GenericFunctions::GenerateNick(string username) {
	string nick = username;
	
	// strip out everything after @
	string::size_type at_pos = nick.find("@");
...
string GenericFunctions::CrushOutWhitespace(string text) {
...
int GenericFunctions::SeparateGroupSpecifiers(string text, string &room, string &server, string &user) {
	// separate room and server
	string::size_type at_pos = text.find("@");
	...
	if (at_pos != string::npos) {
		...
		string::size_type slash_pos = text.find("/");
